...
module ufo_gnssro_bndnbam_util_mod
  ...
  use ufo_utils_mod, only: cmp_strings ! MJM
...
! -------------------------------------------------------------------------------
  ...
  logical, intent(in)            :: super_ref_ECMWF ! MJM
...
!      Compute derivative at new grids (dN/ds) using Lagrange interpolators
       ...
       !if (super_ref_GEOS .or. cmp_strings(super_ref_qc, "ECMWF")) then ! MJM
       if (super_ref_GEOS .or. super_ref_ECMWF) then ! MJM
