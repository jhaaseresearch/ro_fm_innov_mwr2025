...
subroutine ufo_gnssro_bndnbam_simobs(self, geovals, hofx, obss)
  ...
  real(kind_real)                         :: kval, max_gradRef_ropp
...
!    ROPP style super refraction check
       ...
       ! Extra SR checks from ROPP1D, MJM 
       ! Max dn_dx set to critical gradient of refrac, MJM
       gradRef = (ref(iobs+1) - ref(iobs))/(refXrad(iobs+1) - refXrad(iobs))
       max_gradRef_ropp =  0.157 ! in N-units/m
       if ( gradRef > max_gradRef_ropp ) then
          qcfail = .true.
          super_refraction_flag(iobs) = 1
          cycle obs_loop
       end if                             
  
       ! Inverse refrac scale height set to critical gradient of refrac, MJM
       kval = LOG(ref(iobs)/ref(iobs+1))/(refXrad(iobs+1) - refXrad(iobs))
       kval = MAX(0.000001, kval)        
       !kval = MAX(1.0e-6_wp, kval)        
       max_gradRef_ropp =  0.157 ! in N-units/m
       if (kval > (max_gradRef_ropp/ref(iobs))) then
          qcfail = .true.
          super_refraction_flag(iobs) = 1
          cycle obs_loop
       end if
