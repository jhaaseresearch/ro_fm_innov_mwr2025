...
if ( nargs == nargs_req ):
	...
	print("")
	print("     Usage:")
	print("       " + script_name + " <arrecon_year> <iop_str> <series_id> <exp_id> <output_root>")
	print("")
	print("     Example:")
	print("       python " + script_name + " 2022 full 05 06 ../")
	print("")
...
#PATH_OBS = input_root_path + '/'+year_str+iop_str+'/series'+series_id+'/'+exp_id+'/'+year_str+'-02-??_????/hofx/'
...
print(len(infile_name_obs_list))
#sys.exit()
...
for ifile in infile_name_obs_list:
	...
	for iid in unique_id_occ:
		...
		hgt_bot = hgt_tmp[0]
	     ...
	     # Need occs over the whole globe 
		print('------------')
		...
		for ibin in range(nbins):
			...
			#print(ba_omb_binned[ibin])
			#print(ba_omb_binned[ibin].dtype)
			#sys.exit()
...
outfile_name = output_path_ascii + \
		'stats_fv3jedi-omb_arrecon'+year_str+iop_str+ \
			'_exp'+series_id+exp_id+'_bin-'+str(inc_hgt_bins_m)+'m_global.txt'
