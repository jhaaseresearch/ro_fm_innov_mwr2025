...
FILE_OBS1 = 'stats_fv3jedi-omb_arrecon' + arrecon_year_str + iop_str + '_exp' + series_id + '0[3456]_bin-' + \
		str(inc_hgt_bins_m) + 'm_{global,tropics,nepac}.txt'
...
for ifile in infile_name_obs_list:
	...
	exp_id = exp_id_raw[-2:]
	region_raw = name_file_split[5]
	region_str_raw = region_raw[0:-4]

	if region_str_raw == 'global':
		region_str = 'Globe'
		occ_dataset_linestyle = 'solid'
	elif region_str_raw == 'tropics':
		region_str = 'Tropics'
		occ_dataset_linestyle = 'dashed'
	elif region_str_raw == 'nepac':
		region_str = 'NE Pacific'
		#region_str = 'NE Pac.'
...
for ifile in range(nfiles):
	label_str_nobs = occ_region_name_list[ifile] + ' ' + occ_dataset_name_list[ifile]
...
output_name_pdf = 'stats_compare_fv3jedi-omb_compare-count-propor-regions-' + str(inc_hgt_bins_m) + 'm.pdf'
